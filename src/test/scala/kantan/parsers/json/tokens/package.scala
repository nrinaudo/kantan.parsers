...
package object tokens {
  ...
  /** Helper for parsing from Seq[Parsed[JsonToken]] into a JsonToken.
    ...
    * produce a JValue, but in some other scenarios, you might want to keep it (if the parsing phase is followed by a
    * type checking one, for example, and you want to provide useful type error messages).
    */
  val jsonToken: TokenParser[JsonToken] = token[Parsed[JsonToken]].map(_.value)

// Parsers for the various "syntax" tokens.
  val fieldSep: TokenParser[Unit] =
    jsonToken.collect { case JsonToken.FieldSep => () }.label(JsonToken.FieldSep.toString)
  val listSep: TokenParser[Unit] = jsonToken.collect { case JsonToken.ListSep => () }.label(JsonToken.ListSep.toString)
  val objStart: TokenParser[Unit] =
    jsonToken.collect { case JsonToken.ObjStart => () }.label(JsonToken.ObjStart.toString)
...
// ---------------------------------------------------------------------------------------------------------------------
  ...
  def jArray: TokenParser[JArray] =
    jValue.repSep0(listSep).between(arrayStart, arrayEnd).map(JArray.fromSeq).label("array")
